...
export function DailyRevenueAccrualPanel({ userId, organizationId }: { userId: string; organizationId: string }) {
  ...
  return (
    <div className="space-y-4 print-section">
      ...
      <p className="text-xs text-muted-foreground print:hidden">{data?.vatNote}</p>
            ...
            <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
              <Card>
                <CardContent className="p-4">
                  <p className="text-sm text-muted-foreground">Period subtotal (earned)</p>
                  <p className="text-2xl font-bold">{formatNaira(totals.subtotal)}</p>
                </CardContent>
              </Card>
              <Card>
                <CardContent className="p-4">
                  <p className="text-sm text-muted-foreground">VAT 7.5%</p>
                  <p className="text-2xl font-bold">{formatNaira(totals.vat)}</p>
                </CardContent>
              </Card>
              <Card>
                <CardContent className="p-4">
                  <p className="text-sm text-muted-foreground">Total incl. VAT</p>
                  <p className="text-2xl font-bold">{formatNaira(totals.withVat)}</p>
                </CardContent>
              </Card>
              <Card>
                <CardContent className="p-4">
                  <p className="text-sm text-muted-foreground">Room-night accrual</p>
                  <p className="text-2xl font-bold">{formatNaira(totals.accommodation)}</p>
                </CardContent>
              </Card>
            </div>
          )}
              ...
              <tbody>
                {(data?.byDay || []).map((row: any) => (
                  <tr key={row.date} className="border-t">
                    ...
                    <td className="text-right p-2">{formatNaira(row.accommodationAccrual)}</td>
                    <td className="text-right p-2">{formatNaira(row.folioChargesRecognized)}</td>
                    <td className="text-right p-2 font-medium">{formatNaira(row.subtotal)}</td>
                    <td className="text-right p-2">{formatNaira(row.vatAmount)}</td>
                    <td className="text-right p-2">{formatNaira(row.totalWithVat)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
...
export function OccupancyRangePanel({ userId, organizationId }: { userId: string; organizationId: string }) {
  const [start, setStart] = useState<Date>(() => new Date())
  const [end, setEnd] = useState<Date>(() => new Date())
  const [loading, setLoading] = useState(false)
  const [data, setData] = useState<any>(null)
  ...
  return (
    <div className="space-y-4 print-section">
      ...
      <p className="text-xs text-muted-foreground">
        Occupancy rate each day uses <strong>occupied ÷ (total rooms − out of order)</strong>. Out-of-order = room status
        &quot;maintenance&quot;.
      </p>
      {s && (
        ...
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          <Card>
            <CardContent className="p-4">
              ...
              <p className="text-xl font-bold">{formatNaira(data.paymentsTotal)}</p>
            </CardContent>
          </Card>
          <Card>
            <CardContent className="p-4">
              <p className="text-xs text-muted-foreground">Transactions</p>
              <p className="text-xl font-bold">{formatNaira(data.transactionsTotal)}</p>
            </CardContent>
          </Card>
          <Card>
            <CardContent className="p-4">
              <p className="text-xs text-muted-foreground">Refunds</p>
              <p className="text-xl font-bold text-red-600">−{formatNaira(data.refundsTotal)}</p>
            </CardContent>
          </Card>
          <Card>
            <CardContent className="p-4">
              <p className="text-xs text-muted-foreground">Net sales collection</p>
              <p className="text-xl font-bold">{formatNaira(data.netSalesCollection)}</p>
            </CardContent>
          </Card>
        </div>
      )}
        ...
        {Object.entries(buckets).map(([k, v]) => (
          <Card key={k}>
            <CardContent className="p-3 flex justify-between items-center">
              ...
              <span className="font-semibold">{formatNaira(Number(v) || 0)}</span>
            </CardContent>
          </Card>
        ))}
      </div>
...
export function RefundsPanel({ userId, organizationId }: { userId: string; organizationId: string }) {
  const [rows, setRows] = useState<any[]>([])
  const [loading, setLoading] = useState(false)
  const [open, setOpen] = useState(false)
  const [guestSearch, setGuestSearch] = useState('')
  const [guestHits, setGuestHits] = useState<{ id: string; name: string; balance: number }[]>([])
  ...
  const load = useCallback(async () => {
    ...
    setLoading(true)
    try {
      const res = await fetch(`/api/refunds?caller_id=${userId}`, { credentials: 'include' })
      const json = await res.json()
      if (!res.ok) {
        toast.error(json.error || 'Failed')
        setRows([])
    ...
    try {
      const res = await fetch('/api/refunds', {
        ...
        body: JSON.stringify({
          ...
          reference_payment_date: refPayDate ? format(refPayDate, 'yyyy-MM-dd') : null,
        }),
      })
      const json = await res.json()
      if (!res.ok) {
            ...
            <tbody>
              {rows.map((r) => (
                <tr key={r.id} className="border-t">
                  ...
                  <td className="text-right p-2 font-medium">{formatNaira(Number(r.amount))}</td>
                  <td className="p-2 text-muted-foreground">{r.reason}</td>
                </tr>
              ))}
            </tbody>
          </table>
          {rows.length === 0 && <p className="p-6 text-center text-muted-foreground text-sm">No refunds in range.</p>}
        </div>
      )}
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>Reduces guest balance and net sales collection for the period.</DialogDescription>
          </DialogHeader>
          <div className="space-y-3">
            <div className="space-y-2">
              <Label>Find guest</Label>
              <div className="flex gap-2">
                <Input value={guestSearch} onChange={(e) => setGuestSearch(e.target.value)} placeholder="Name…" />
                <Button type="button" variant="secondary" onClick={() => searchGuests()}>
                  Search
                </Button>
              </div>
              <div className="max-h-32 overflow-y-auto border rounded">
                {guestHits.map((g) => (
                  <button
                    key={g.id}
                    type="button"
                    className={cn('w-full text-left px-2 py-1.5 text-sm hover:bg-muted', guestId === g.id && 'bg-muted')}
            ...
            <div className="space-y-2">
              ...
              <Input value={bookingId} onChange={(e) => setBookingId(e.target.value)} placeholder="UUID" />
            </div>
            <div className="space-y-2">
            ...
            <div className="grid grid-cols-2 gap-2">
              <div>
                ...
                <DatePick date={refundDate} onSelect={(d) => d && setRefundDate(d)} />
              </div>
              <div>
                <Label>Original payment date (optional)</Label>
                <DatePick date={refPayDate} onSelect={setRefPayDate} label="Pick" />
              </div>
            </div>
          </div>
          <DialogFooter>
            ...
            <Button onClick={() => void submitRefund()} disabled={saving}>
              {saving ? <Loader2 className="h-4 w-4 animate-spin" /> : 'Save refund'}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
