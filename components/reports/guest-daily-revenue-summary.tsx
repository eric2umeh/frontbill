...
function classifyRoomCharge(chargeType: string, description: string): boolean {
  ...
  if (
    t === "charge" &&
    (/\broom\b/i.test(description || "") || d.includes("nightly"))
  )
    return true;
  return false;
}
...
export function GuestDailyRevenueSummary({
  ...
  const selectedGuestLabel =
    guestOptions.find((g) => g.id === guestId)?.name ?? null;
  ...
  const runReport = useCallback(async () => {
    ...
    try {
      ...
      if (be) throw be;
      const bookingIds = [
        ...new Set((dayBookings || []).map((b: any) => b.id)),
      ];
      const folios = [
        ...new Set(
          (dayBookings || []).map((b: any) => b.folio_id).filter(Boolean),
        ),
      ] as string[];
      ...
      if (bookingIds.length === 0) {
        ...
        setFolioIds([]);
        toast.message(
          "No in-house booking for this guest on the selected date",
        );
      ...
      <div className="flex flex-wrap items-end gap-3 print:hidden">
        <Popover open={guestOpen} onOpenChange={setGuestOpen}>
          <PopoverTrigger asChild>
            <Button
              variant="outline"
              className={cn("min-w-[240px] justify-between font-normal")}
            >
          ...
          <PopoverContent
            className="w-[min(380px,var(--radix-popover-content-available-width,380px))] p-2"
            align="start"
          >
            <Input
        ...
        <Button
          onClick={() => runReport()}
          disabled={loadingReport || !guestId}
        >
          {loadingReport ? (
      ...
      <div className="rounded-lg border bg-white dark:bg-background p-4 md:p-6 text-black dark:text-foreground shadow-sm">
        ...
        <div className="text-center border-b pb-4 mb-4">
          ...
          <p className="text-xs mt-1 text-muted-foreground print:text-gray-700">
            Date Range: From {dateStrNg} to {dateStrNg} · Currency: NGN · VAT
            shown at 7.5% inclusive (default)
          </p>
          <p className="text-[11px] mt-2 text-muted-foreground print:text-gray-700">
            POS-style lines (misc. descriptions) grouped as Added Charges
            pending dedicated restaurant/laundry codes.
          </p>
        </div>
          ...
          <>
            <div className="overflow-x-auto">
              <table className="w-full text-xs md:text-sm border-collapse border border-neutral-800 print:border-neutral-900">
                ...
                <tbody>
                  {(["Front Desk", "Misc Income", "POS Revenue"] as const).map(
                    (section) => {
                      const bucket = segments.rows.filter(
                        (r) => r.section === section,
                      );
                      return (
                        <FragmentBlock
              ...
              <table className="w-full text-sm">
                <tbody>
                  ...
                  <tr className="border-b">
                    ...
                    <td className="px-3 py-2 text-right">
                      {formatNaira(addedChargesGross)}
                    </td>
                  </tr>
                  <tr className="bg-neutral-50 font-semibold">
                    <td className="px-3 py-2">Ground Total</td>
                    <td className="px-3 py-2 text-right">
                      {formatNaira(segments.totalGross)}
                    </td>
                  </tr>
                </tbody>
              </table>
  ...
  const body = rows.map((r) => {
    ...
    return (
      <tr key={`${section}-${r.label}`}>
        <td className="border border-neutral-400 px-2 py-1.5 pl-4">
          {r.label}
        </td>
        <td className="border border-neutral-400 px-2 py-1.5 text-right">
          {formatMoneyExact(r.preTax)}
        </td>
        <td className="border border-neutral-400 px-2 py-1.5 text-right">
          {formatMoneyExact(r.vat)}
        </td>
        <td className="border border-neutral-400 px-2 py-1.5 text-right">
          {formatMoneyExact(0)}
        </td>
  ...
  return (
    <>
      <tr className="bg-neutral-50/90">
        <td
          colSpan={9}
          className="border border-neutral-400 px-2 py-1.5 font-bold"
        >
          {section}
        </td>
      </tr>
      ...
      <tr className="font-semibold bg-neutral-50/50">
        ...
        <td className="border border-neutral-400 px-2 py-1.5 text-right">
          {formatMoneyExact(subPre)}
        </td>
        <td className="border border-neutral-400 px-2 py-1.5 text-right">
          {formatMoneyExact(subVat)}
        </td>
        <td className="border border-neutral-400 px-2 py-1.5 text-right">
          {formatMoneyExact(0)}
        </td>
