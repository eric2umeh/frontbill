...
import {
  ...
  AlertCircle, Download, CalendarDays, Banknote,
...
export function RevenuePanel() {
  ...
  const dateRange = useMemo(() => {
    ...
    switch (period) {
      case 'today': {
        ...
        return { from: new Date(b.startIso), to: new Date(b.endInclusiveIso) }
      }
      case '7d':
      ...
      case 'custom': {
        const b = hotelCalendarDayUtcBounds(format(customDate, 'yyyy-MM-dd'), tz)
        return { from: new Date(b.startIso), to: new Date(b.endInclusiveIso) }
      }
    }
  }, [period, customDate])
  ...
  const fetchData = async () => {
    ...
    const [paymentsRes, txRes, bookingsRes, ledgerRes] = await Promise.all([
      supabase
        .from('payments')
        .select(`id, amount, payment_method, payment_date, booking_id, guest_id, notes, reference_number,
    ...
    const visibleTx = (txRes.data || []).filter((t: { status?: string | null }) => {
      const st = String(t.status || '').toLowerCase()
      return st !== 'void' && st !== 'cancelled'
    })
    const outletNums = collectOutletPaidTransactionOrderNumbers(visibleTx)
    const payDeduped = filterDuplicatePaymentRows(
      (paymentsRes.data || []).filter((p: { notes?: string | null }) =>
        !shouldHideOutletPaymentDuplicate((p as any).notes, outletNums),
      ),
      visibleTx,
    )

    const fromTx = visibleTx.map((t: any) => ({
      id: t.id,
      ...
      guest_id: null,
      guests: { name: t.guest_name || 'Unknown' },
      bookings: {
        folio_id: t.transaction_id || '—',
        check_in: '',
        check_out: '',
        payment_status: String(t.status || 'paid').toLowerCase() === 'paid' ? 'paid' : 'partial',
      },
    }))

    setPayments([...payDeduped, ...fromTx])
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground text-sm">Collections by method, folio, and booking payment status</p>
        </div>
