...
export function PurchasingWorkspace() {
  ...
  const actualSpent = useMemo(
    () => retireLines.filter((l) => !l.removed).reduce((s, l) => s + l.totalPaid, 0),
    [retireLines],
  )
      ...
      <div className="space-y-6">
        ...
        <div className="rounded-lg border border-blue-200 bg-blue-50/50 dark:bg-blue-950/20 p-4 flex gap-3 text-sm">
          ...
          <div>
            ...
            <p className="text-muted-foreground">
              After manager approval and cash disbursement, retire the PO at market. Edit actual
              quantities and prices. Stock is updated in central store when retirement is submitted.
            </p>
          </div>
        </div>
        ...
        <div className="space-y-2">
          {retireCandidates.length === 0 && (
            <p className="text-sm text-muted-foreground">
              No POs ready for market retirement yet — complete accountant and manager approvals first.
            </p>
          )}
          {retireCandidates.map((po) => (
            <div key={po.id} className="flex justify-between items-center rounded-lg border p-4">
              <div>
                <p className="font-medium">{po.weekLabel}</p>
                <p className="text-sm text-muted-foreground">
                  {po.createdByName} · Disbursed: {formatNaira(po.cashDisbursed)}
                </p>
              </div>
  ...
  return (
    <div className="space-y-6">
      <Button variant="ghost" onClick={() => setSelectedId(null)}>← Change PO</Button>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        ...
        <StatCard label="Price changes" value={String(retireLines.filter((l) => l.poPrice !== l.actualPrice).length)} />
      </div>
      <Table>
        <TableHeader>
          <TableRow>
        ...
        <TableBody>
          {retireLines.filter((l) => !l.removed).map((line) => (
            <TableRow key={line.lineId}>
              <TableCell>{line.name} <Badge className="ml-1 bg-emerald-100 text-emerald-800 text-[10px]">OK</Badge></TableCell>
              ...
              <TableCell className="text-right">{formatNaira(line.poPrice)}</TableCell>
              <TableCell className="text-right">
                <Input
                  type="number"
              ...
              <TableCell className="text-right">{formatNaira(line.totalPaid)}</TableCell>
              <TableCell>
                <Button size="icon" variant="ghost" onClick={() => setRetireLines((p) => p.map((l) => l.lineId === line.lineId ? { ...l, removed: true } : l))}>
...
function StatCard({ label, value, highlight }: { label: string; value: string; highlight?: boolean }) {
  return (
    <div className={`rounded-xl border p-4 ${highlight ? 'ring-2 ring-primary' : ''}`}>
