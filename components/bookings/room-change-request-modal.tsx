...
type RoomRow = { id: string; room_number: string; room_type: string; status: string }
  ...
  useEffect(() => {
    ...
    ;(async () => {
      ...
      try {
        ...
        const filtered = currentRoomId ? rows.filter((r) => r.id !== currentRoomId) : rows
    ...
    <Dialog open={open} onOpenChange={(o) => !o && onClose()}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>
            Guest is in <span className="font-medium text-foreground">{currentRoomLabel}</span>. A Superadmin,
            Administrator, or Manager must approve before the move is applied. Dates and rates stay the same.
          </DialogDescription>
        </DialogHeader>
        <div className="space-y-4 py-2">
          <div className="text-xs text-muted-foreground">
            Stay {overlapSet.cin} → {overlapSet.cout} (only available rooms are listed)
          </div>
              ...
              <Select value={toRoomId} onValueChange={setToRoomId}>
                ...
                <SelectContent>
                  {selectableRooms.length === 0 ? (
                    <div className="px-2 py-3 text-sm text-muted-foreground">No available rooms</div>
                  ) : (
        ...
        <DialogFooter className="gap-2 sm:gap-0">
          ...
          <Button type="button" onClick={() => void handleSubmit()} disabled={submitting || loadingRooms}>
            {submitting ? (
              <>
