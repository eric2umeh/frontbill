...
export function AddChargeModal({
  ...
  const handleAddCharge = async () => {
    ...
    try {
            ...
            if (!selectedLedger?.id && guestRow.name) {
              ...
              if (existingAcct) {
                ...
                await supabase
                  .from("city_ledger_accounts")
                  .update({
                    balance: (existingAcct.balance || 0) + chargeAmount,
                  })
          ...
          if (ledgerType === "organization") {
            ...
            if (orgRow) {
              await supabase
                ...
                .update({
                  current_balance:
                    ((orgRow.current_balance as number) || 0) + chargeAmount,
                })
                .eq("id", selectedLedger.id);
            }
          }
      ...
      toast.success(
        `Charge of ${formatNaira(chargeAmount)} added${isPaidNow ? " (paid)" : " — added to Bill Balance"}`,
      );
      onClose();
      resetForm();
    } catch (error: any) {
  ...
  return (
    ...
    >
      <DialogContent
        className={cn(dialogScrollableContentClass, "sm:max-w-xl")}
      >
        <DialogScrollableHeader>
          <DialogTitle>Add Charge - {booking.folioId}</DialogTitle>
          <DialogDescription className="sr-only">
            Add miscellaneous charge to booking
          </DialogDescription>
        </DialogScrollableHeader>
                    ...
                    <CardContent className="p-3 text-sm">
                      <div className="flex justify-between items-center">
                        ...
                        <span className="font-medium">
                          {selectedLedger?.name || booking.guestName}
                        </span>
                      </div>
                    </CardContent>
                  </Card>
                    ...
                    <Select
                      ...
                      onValueChange={async (id) => {
                        ...
                        try {
                          ...
                          setOrgSearchTerm(
                            resolved.name || resolved.account_name,
                          );
                        } catch (error: any) {
                          toast.error(
