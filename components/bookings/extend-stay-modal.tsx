...
export function ExtendStayModal({
  ...
  const createNewOrganizationAccount = async () => {
    ...
    try {
      ...
      setNewOrgPhone("");
      toast.success(
        `Organization account "${account.name}" created and selected`,
      );
    } catch (error: any) {
  ...
  const handleDiscountRequest = async () => {
    if (!newCheckOutDate || !paymentMethod || paymentMethod === "city_ledger") {
      toast.error(
        "Discount requests use cash, POS, or transfer only (not city ledger in this version)",
      );
      return;
    }
    ...
    if (!Number.isFinite(disc) || disc <= 0 || disc >= additionalAmount) {
      toast.error(
        `Discounted total must be greater than 0 and less than standard ${formatNaira(additionalAmount)}`,
      );
      return;
    }
    if (!userId) {
  ...
  const handleExtend = async () => {
    ...
    if (!booking.id) {
      toast.error(
        "Booking ID is missing. Please close and reopen this dialog.",
      );
      return;
    }
    ...
    try {
      ...
      const { error: chargeError } = await insertFolioCharges(supabase, [
        chargeData,
      ]);
              ...
              <div className="space-y-2 rounded-lg bg-muted p-3 text-xs sm:p-4 sm:text-sm">
                <div className="flex justify-between gap-2">
                  ...
                  <span className="font-semibold">
                    {newCheckOutDate && format(newCheckOutDate, "PP")}
                  </span>
                </div>
                      ...
                      <button
                        ...
                        onClick={() =>
                          setSelectedLedger({
                            ...
                            balance: booking.guestBalance || 0,
                          })
                        }
                            ...
                            <button
                              ...
                              onClick={async () => {
                                try {
                                  const supabase = createClient();
                                  const resolved =
                                    await resolveOrganizationLedgerAccount(
                                      supabase,
                        ...
                        <div className="space-y-2 rounded-md border bg-muted/30 p-3">
                          ...
                          <div className="flex justify-end gap-2">
                            ...
                            <Button
                              ...
                              size="sm"
                              className="h-8 text-xs"
                              onClick={createNewOrganizationAccount}
          ...
          {newCheckOutDate && additionalNights > 0 && (
            <div className="mt-3 space-y-2 rounded-lg border border-amber-200 bg-amber-50/80 p-3 text-xs sm:text-sm">
              <label className="flex items-start gap-2 cursor-pointer">
                ...
                <span>
                  <span className="font-medium">
                    Request discounted extension
                  </span>{" "}
                  — requires Superadmin / Administrator / Manager approval.
                  Standard total {formatNaira(additionalAmount)}.
                </span>
              </label>
              {wantDiscountRequest && (
