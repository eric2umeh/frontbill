...
test('overstay checked-in guests still occupy for outlets and reconcile', async () => {
  const {
    pickOccupyingBooking,
    pickRoomStatusBooking,
    reconcileRoomStatusesForOrganization,
  } = loadRoomOccupancyModule()
  const overstay = {
    id: 'booking-overstay',
    room_id: 'room-105',
    status: 'checked_in',
  ...
  assert.equal(pickOccupyingBooking([overstay])?.id, 'booking-overstay')
  assert.equal(pickRoomStatusBooking([overstay])?.id, 'booking-overstay')

  const updates = []
  const supabase = createMockSupabase({
    updates,
    rooms: [{ id: 'room-105', status: 'occupied' }],
    bookings: [overstay],
  })
  const result = await reconcileRoomStatusesForOrganization(supabase, 'org-1')
  assert.equal(result.updated, 0)
...
test('effective status keeps future reserved and cleaning blocks', () => {
  const { computeEffectiveRoomStatus, pickRoomStatusBooking } =
    loadRoomOccupancyModule()
  const futureBooking = {
    id: 'booking-future',
    room_id: 'room-106',
  ...
  assert.equal(
    computeEffectiveRoomStatus(
      'reserved',
      pickRoomStatusBooking([futureBooking]),
    ),
    'reserved',
  )
  assert.equal(computeEffectiveRoomStatus('cleaning', null), 'cleaning')
})
