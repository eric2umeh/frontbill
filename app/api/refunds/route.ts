...
export async function GET(request: Request) {
  try {
    ...
    if (!canProcessRefunds(prof.role)) {
      return NextResponse.json(
        { error: "You do not have access to refunds" },
        { status: 403 },
      );
    }
  ...
  try {
    ...
    if (!caller_id || !guest_id || !amount || !reason?.trim() || !refund_date) {
      return NextResponse.json(
        {
          error:
            "caller_id, guest_id, amount, reason and refund_date are required",
        },
        { status: 400 },
      );
    }
    ...
    if (!canProcessRefunds(prof.role)) {
      return NextResponse.json(
        { error: "You are not allowed to record refunds" },
        { status: 403 },
      );
    }
    ...
    if (ge || !guest || guest.organization_id !== orgId) {
      return NextResponse.json(
        { error: "Guest not found in this hotel" },
        { status: 404 },
      );
    }
    ...
    if (booking_id) {
      ...
      if (be || !bk || bk.organization_id !== orgId) {
        return NextResponse.json(
          { error: "Booking not found in this hotel" },
          { status: 404 },
        );
      }
    }
      ...
      .insert([
        {
          ...
          reference_payment_date: reference_payment_date
            ? String(reference_payment_date).slice(0, 10)
            : null,
    ...
    if (insE) {
      if (
        /refunds/i.test(insE.message) &&
        /does not exist/i.test(insE.message)
      ) {
        return NextResponse.json(
          {
            error:
              "Refunds table missing — run scripts/040_reporting_refunds_discount_category.sql",
          },
          { status: 503 },
        );
      }
