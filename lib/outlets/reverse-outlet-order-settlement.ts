...
async function reverseCityLedgerBalances(
  ...
  if (acct) {
    const newBal = Math.max(
      0,
      roundMoney((Number(acct.balance) || 0) - input.amount),
    );
    await admin
      .from("city_ledger_accounts")
...
export async function reverseOutletOrderSettlement(
  ...
  const amount = complimentary
    ? 0
    : Math.max(0, Number(input.order.subtotal) || 0);
  ...
  if (
    amount > 0 &&
    isCityLedgerMethod(paymentMethod) &&
    input.order.city_ledger_account_id
  ) {
