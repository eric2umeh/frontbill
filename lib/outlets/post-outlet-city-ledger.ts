...
export async function postOutletCityLedgerCharge(
  ...
  const txDescription =
    `${departmentLabel} — ${orderNumber} — ${lineDetail}`.slice(0, 500);
  ...
  if (bookingId) {
    const { data: bk, error: be } = await supabase
      .from("bookings")
      .select(
        "id, guest_id, balance, payment_status, guests(name), rooms(room_number)",
      )
      .eq("id", bookingId)
  ...
  if (presetLedgerId) {
    ...
    throw new Error(
      "Guest name, room number with active check-in, or city ledger account is required",
    );
  }
    ...
    if (presetLedger.account_type === "organization") {
      ...
      if (orgRow) {
        await supabase
          .from("organizations")
          .update({
            current_balance: (Number(orgRow.current_balance) || 0) + amount,
          })
          .eq("id", presetLedger.id);
      }
    }
