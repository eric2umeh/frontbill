...
export async function createBookingFromPayload(
  ...
  if (!isCityLedger) {
    if (payment_status === "paid") {
      paidAmount = pay_above_room_total
  ...
  if (isCityLedger && !ledger_account) {
    return {
      ok: false,
      error: "City ledger booking missing ledger account id",
    };
  }
  ...
  if ((overlaps?.length ?? 0) > 0) {
    return {
      ok: false,
      error: "Selected room is already booked for these dates",
    };
  }
  ...
  if (be || !booking)
    return { ok: false, error: be?.message || "Booking insert failed" };
  ...
  if (payment_method === "city_ledger" && ledger_account) {
    ...
    const acctType =
      acc?.account_type ||
      (ledger_tab === "individual" ? "individual" : "organization");
    if (acctType !== "individual" && acctType !== "guest") {
      ...
      if (orgRow) {
        await admin
          .from("organizations")
          .update({
            current_balance: (Number(orgRow.current_balance) || 0) + total,
          })
          .eq("id", ledger_account);
      }
    }
