...
export function splitOutletBillVat(
  grandTotal: number,
  vatRate = OUTLET_BILL_VAT_RATE,
): {
  billAmount: number;
  vatAmount: number;
  grandTotal: number;
...
export function buildOutletThermalBillPayload(input: {
  ...
  return {
    ...
    paymentMethod: input.isComplimentary
      ? "complimentary"
      : input.paymentMethod,
...
export function buildOutletThermalBillHtml(
  p: OutletThermalBillPayload,
): string {
  const dt = formatReceiptDateTime(p.printedAtIso);
  const [datePart, timePart] = dt.includes(",")
