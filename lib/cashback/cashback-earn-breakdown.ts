...
function parseRateFromDescription(
...
export function groupEarnTransactionsByRate(
  ...
  for (const t of transactions) {
    ...
    const stored =
      t.earn_rate_percent != null ? Number(t.earn_rate_percent) : NaN;
    const rate = Number.isFinite(stored)
      ? stored
      : parseRateFromDescription(t.description);
    const key =
      rate != null
        ? `rate:${rate}`
        : `unknown:${String(t.description || "").slice(0, 40)}`;
    const label =
