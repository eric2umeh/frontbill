...
/**
 * Enforce that body/query `caller_id` matches the authenticated session.
 * Returns a 401 response when the check fails; otherwise null.
 */
export async function unauthorizedUnlessCallerMatches(
  request: Request,
  callerId: string | null | undefined,
): Promise<NextResponse | null> {
  const authed = await resolveAuthedUserId(request)
  if (!callerMatchesSession(callerId, authed)) {
