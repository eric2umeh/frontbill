...
function isActiveRoomHold(booking: RemainingRoomBooking): boolean {
  ...
  return (
    ACTIVE_ROOM_HOLD_STATUSES.includes(status) &&
    !CLOSED_FOLIO_STATUSES.includes(folioStatus)
  )
}

/** Pure helper for cancel room-status decisions (also used by unit tests). */
export function roomStatusAfterReservationCancel(
  ...
  if (activeHolds.some((booking) => normStatus(booking.status) === 'checked_in')) {
    return 'occupied'
  }
...
export async function cancelBookingReservation(
  ...
  if (roomId) {
    const [{ data: room, error: roomFetchErr }, { data: remainingBookings, error: bookingFetchErr }] =
      await Promise.all([
    ...
    const nextRoomStatus = roomStatusAfterReservationCancel(
      remainingBookings ?? [],
      room?.status,
    )
