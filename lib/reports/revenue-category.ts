...
export function parseRevenueCategory(raw: string | null | undefined): RevenueDepartment {
...
export function inferRevenueCategory(
  ...
  if (['room_charge', 'extended_stay', 'reservation'].includes(t)) return 'accommodation'
  if (t === 'folio_note') return 'other'

  if (/\brebecca\b/.test(d) || /\brebecca hall\b/.test(d)) return 'hall_rebecca'
  if (/\bfloxy\b/.test(d) || /\bfloxy hall\b/.test(d)) return 'hall_floxy'
  if (/\bboard room\b/.test(d) || /\bboardroom\b/.test(d)) return 'hall_board_room'
  if (/\bbanquet\b/.test(d) || /\bhall\b/.test(d) || /\bevent\b/.test(d)) return 'events'

  if (/\brestaurant\b/.test(d) || /\bdinner\b/.test(d) || /\blunch\b/.test(d) || /\bbreakfast\b/.test(d) || /\bfood\b/.test(d))
    return 'restaurant'
  if (/\bbar\b/.test(d) || /\bdrinks?\b/.test(d) || /\bbeverage\b/.test(d) || /\bminibar\b/.test(d)) return 'bar'
